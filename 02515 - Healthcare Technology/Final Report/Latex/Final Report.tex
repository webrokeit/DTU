...
\end{tabular}
\end{table}
Overweight and obesity are major risk factors for a number of chronic diseases. These diseases include diabetes, cardiovascular diseases and cancer. In Denmark the total expenses linked with obesity and the treatment thereof in hospitals was calculated as being 1.1 billion DKK in 2004 \cite{consequences-obese}.
...
The Workout Memory Game, as the name suggests, is a game where both memory and physical exercises are key components. The game consists of rounds, where in each round you have to perform certain exercises. The exercises are Squat and Jump. These exercises were chosen due to the ability to build muscle mass and increase heart rate when done repeatedly \cite{livestrong-squat}\cite{livehealthy-jump}. Round 1 consists of 1 exercise, Round 2 consists of 2 exercises - the previous one and a new exercise chosen at random and the game continues like this until game over.
...
\section{Scene}
The scene was kept simple. There is a skybox with a solid color used as the background for the game, though when the Kinect is attached it will display the feed from the Kinect's RGB camera. GUIText objects are then used to display various texts. The game is to be kept simple and not overly fancy, therefore the texts are using a font with the look and feel of good old 8bit games. The font is readable close by and when it's scaled up. The font is called \emph{Press Start 2P} and is Open Font licensed (OFL) \cite{2pfont}. 
...
\bibitem{livestrong-squat}
	Tony Gentilcore, \emph{Why Squats are Essential to a Stronger and Healthier Body}
	\url{http://bit.ly/1vKlPUV}

\bibitem{livehealthy-jump}
	Van Thompson, \emph{Why Does the Number of Jumping Jacks You Do Increase Your Heart Rate?}
	\url{http://bit.ly/1vKlRwi}
